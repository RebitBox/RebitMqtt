...
function forceRestartScanner() {
  ...
  // Step 1: Properly stop the GlobalKeyboardListener
  if (state.globalKeyListener) {
    try {
      // Call kill() which calls stop() → proc.kill()
      state.globalKeyListener.kill();
    } catch (e) { /* ignore */ }
    state.globalKeyListener = null;
  }

  // Step 2: Force kill ALL WinKeyServer.exe instances
  // This ensures UnhookWindowsHookEx is called and hooks are released
  // answering manufacturer Q1: ensure hook is deregistered properly
  const { exec } = require('child_process');
  exec('taskkill /F /IM WinKeyServer.exe /T', (err, stdout) => {
    if (stdout && stdout.trim()) log('Killed WinKeyServer: ' + stdout.trim(), 'qr');
    // Step 3: Wait for hook cleanup, then restart fresh
...
function createKeyboardListener() {
  ...
  // Kill any lingering WinKeyServer.exe before starting new one
  // Prevents multiple hooks accumulating — answers manufacturer Q2
  try {
    require('child_process').execSync('taskkill /F /IM WinKeyServer.exe /T', { timeout: 2000 });
    log('Cleared old WinKeyServer instances', 'qr');
  } catch (_) { /* none running — that is fine */ }
...
async function resetSystemForNextUser(forceStop = false) {
  ...
  try {
    ...
    // ⚠️ Keep isReady = false until QR scanner is fully restarted

    // ✅ STEP 1: Kill ALL WinKeyServer instances (cleanup hooks/handles)
    log('🔄 Killing all WinKeyServer instances...', 'qr');
    if (state.globalKeyListener) {
      try { state.globalKeyListener.kill(); } catch (_) {}
      state.globalKeyListener = null;
    }
    try {
      require('child_process').execSync('taskkill /F /IM WinKeyServer.exe /T', { timeout: 3000 });
      log('✅ WinKeyServer killed — hooks released', 'qr');
    } catch (_) { /* none running */ }

    // ✅ STEP 2: Reset USB device (fix motor noise corruption)
    log('🔄 Resetting USB QR scanner device...', 'qr');
    resetUSBDevice().then(() => {

      // ✅ STEP 3: Wait for USB to re-enumerate
      setTimeout(() => {
        log('🔄 Starting fresh QR listener...', 'qr');
        setupQRScanner();

        // ✅ STEP 4: Only NOW mark system as ready
        setTimeout(() => {
          state.isReady = true;
          ...
          log('✅ System fully ready — QR scanner active', 'success');
          log('📱 Ready — scan QR code now', 'qr');
          log('⏱️ Reset completed in ' + (Date.now() - resetStartTime) + 'ms', 'perf');

        }, 2000); // 2s for WinKeyServer to fully initialize

      }, 2000); // 2s for USB re-enumeration
    });
  }
}
...
function gracefulShutdown() {
  ...
  // Force kill ALL WinKeyServer.exe instances on shutdown
  // Ensures UnhookWindowsHookEx is called — answers manufacturer Q1, Q2, Q3
  try {
    require('child_process').execSync('taskkill /F /IM WinKeyServer.exe /T', { timeout: 3000 });
    log('WinKeyServer.exe terminated on shutdown', 'qr');
